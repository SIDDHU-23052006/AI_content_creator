...
function buildPrompt(type, data, profile) {
  ...
  const mergedData = {
    topic: data.topic,
    tone: data.tone || profile.defaultTone || "Professional",
    audience: data.audience || profile.industry || "General audience",
    keywords: data.keywords || "",
    length: data.length || "Medium",
    writingStyle: profile.writingStyle || "Neutral",
  };
...
User Preferences:
- Default Tone: ${profile.defaultTone || "Professional"}
- Industry: ${profile.industry || "General"}
- Writing Style: ${profile.writingStyle || "Neutral"}
`;
...
`;
  ...
  ? chatId
  : createChatIfNotExists(null, topic, type);
